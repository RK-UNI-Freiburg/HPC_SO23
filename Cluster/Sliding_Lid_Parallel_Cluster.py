...
if len(sys.argv) != 3:
    print("Usage: python3 Sliding_Lid_Parallel_Cluster.py <ux_file_path> <uy_file_path>")
    sys.exit(1)
    ...
    for i in range(9):
        ...
        if boundary[3] == True:
            if i in index_north:
                ...
                f_inm[index_south[np.where(i == index_north)[0][0]], 0, :] = f_inm_dummy[i, 0, :] - 2 * w_i[
                    i] * rho_average * ((c_ai[:, i] @ u_w).T / (c_s ** 2))
...
save_mpiio(cartcomm, UX_FILE, u_anm[0, 1:-1, 1:-1])
save_mpiio(cartcomm, UY_FILE, u_anm[1, 1:-1, 1:-1])
